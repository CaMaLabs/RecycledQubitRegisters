#!/usr/bin/env python3
"""Matched same-job IBM Quantum comparison for compiled toy Shor N=21, a=2.

This experiment is deliberately harder than the N=15 benchmark:
  * true order r=6 rather than a power of two,
  * ideal QPE peaks generally lie between binary grid points,
  * every U^(2^k) modular-power round is nontrivial.

To keep the real-QPU circuit within NISQ depth, the six-state multiplicative
orbit of 2 mod 21 is encoded into a 3-qubit work register:

  exponent label 0..5  <->  {1,2,4,8,16,11} mod 21.

Multiplication by 2 mod 21 is therefore an exact +1 mod 6 permutation on the
encoded orbit. U^(2^k) is +2^k mod 6. States |6> and |7> are left unused.

This is a compiled orbit encoding, not a generic 5-qubit modular multiplier.
It is a real coherent work register and the measured phase is not injected.

Wide and recycled circuits:
  * execute in the same SamplerV2 job,
  * start with the same three physical work qubits,
  * use one shared physical neighborhood,
  * record layout/calibration/resource metadata.
...
SCRIPT_REVISION = "2026-09-13-shor21-matched-v1"
N = 21
A = 2
TRUE_ORDER_FOR_VALIDATION_ONLY = 6
WORK_BITS = 3
ORBIT = [1, 2, 4, 8, 16, 11]
...
def append_cccx(qc, c0, c1, c2, target):
    qc.append(MCXGate(3), [c0, c1, c2, target])


def apply_controlled_shift_mod6(qc, control, work, delta: int):
    """Exact controlled +delta mod 6 on states 0..5, leaving 6 and 7 fixed.

    Short exact 3-qubit permutation decompositions were exhaustively found
    from X/CX/CCX gates and then promoted by one external phase control.
    """
    w0, w1, w2 = work
    delta %= 6
    if delta == 0:
        return
    if delta == 1:
        qc.ccx(control, w0, w2)
        append_cccx(qc, control, w0, w2, w1)
        append_cccx(qc, control, w0, w1, w2)
        qc.cx(control, w0)
        append_cccx(qc, control, w1, w2, w0)
        return
    if delta == 2:
        qc.cx(control, w2)
        qc.ccx(control, w2, w1)
        qc.ccx(control, w1, w2)
        return
    if delta == 4:
        qc.cx(control, w1)
        qc.ccx(control, w1, w2)
        qc.ccx(control, w2, w1)
        return
    raise ValueError(f"Unsupported mod-6 shift {delta}; expected 0,1,2,4")


def _uncontrolled_map_for_delta(delta: int) -> list[int]:
    return [((x + delta) % 6) if x < 6 else x for x in range(8)]
...
def _apply_uncontrolled_gate_sequence(delta: int, x: int) -> int:
    ...
    def cx(v, c, t):
        return xgate(v, t) if bit(v, c) else v

    def ccx(v, c0, c1, t):
        return xgate(v, t) if bit(v, c0) and bit(v, c1) else v
...
def self_test_compiled_orbit() -> dict:
    details = {}
    for delta in (1, 2, 4):
        got = [_apply_uncontrolled_gate_sequence(delta, x) for x in range(8)]
...
def build_recycled(phase_bits: int, use_measure2: bool):
    ...
    qc = QuantumCircuit(q, phase, name=f"shor21_recycled_{phase_bits}b")
    anc = q[0]
    work = list(q[1:])
    ...
    for k in range(phase_bits - 1, -1, -1):
        ...
        qc.reset(anc)
        qc.h(anc)
        apply_controlled_shift_mod6(qc, anc, work, delta_for_power(k))
...
def build_wide(phase_bits: int):
    ...
    phase = ClassicalRegister(phase_bits, "phase")
    qc = QuantumCircuit(phase_q, work_q, phase, name=f"shor21_wide_{phase_bits}b")

    for k in range(phase_bits):
        qc.h(phase_q[k])
        apply_controlled_shift_mod6(qc, phase_q[k], list(work_q), delta_for_power(k))
...
def ideal_metrics(phase_bits: int):
    ...
    return {
        ...
        "top": [
            ...
            for y, p in sorted(enumerate(probs), key=lambda kv: kv[1], reverse=True)[:16]
        ],
    }
...
def choose_shared_plan(backend, phase_bits: int):
    ...
    for anc in ancillas:
        ...
        for work_tuple in itertools.permutations(others, WORK_BITS):
            ...
            local = work_cost + 0.25 * control_cost + 0.06 * qft_cost
...
def main():
    ap = argparse.ArgumentParser(description="Matched real-QPU compiled Shor test: N=21, a=2, r=6.")
    ...
    ap.add_argument("--max-execution-time", type=int, default=90)
    ap.add_argument("--transpile-only", action="store_true")
    ap.add_argument("--self-test", action="store_true")
    ap.add_argument("--outdir", type=Path, default=Path("results/ibm_shor21"))
    args = ap.parse_args()
    ...
    if args.phase_bits < 4:
        raise SystemExit("Use at least 4 phase bits for the r=6 N=21 test; 6 is recommended.")
    ...
    out_base = {
        ...
        "scope_note": (
            "This is an exact compiled six-state orbit encoding for N=21,a=2, not a "
            "generic 5-qubit modular multiplier. Same initial physical work sites are "
    ...
    if args.transpile_only:
        path = args.outdir / f"ibm_shor21_matched_transpile_{args.phase_bits}b.json"
    ...
    path = args.outdir / f"ibm_shor21_matched_{args.phase_bits}b_{stamp}.json"
