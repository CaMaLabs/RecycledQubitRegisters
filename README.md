...
### First end-to-end real-QPU factoring run

The repository now also contains a real-IBM Shor demonstration for `N=15`, `a=2` in which the phase is generated by an actual 4-qubit modular work register rather than injected as a known phase. A recycled phase ancilla is measured, reset, and reused over four QPE rounds.

On `ibm_fez`, the transpiled 5-logical-qubit circuit had depth **181** and **55 CZ gates**. In 256 shots, the order-4 spectrum produced `0100` 53 times, `1000` 57 times, and `1100` 49 times. Those 159 informative shots recovered `r=4` and factors `(3,5)`, for an observed per-shot factor-recovery probability of **62.11%**. The aggregate factorization succeeded.

This is a canonical toy Shor instance with a specialized modular-multiplication network, not a claim that cryptographic RSA sizes are tractable on current hardware.

See [`results/hardware/RESULTS.md`](results/hardware/RESULTS.md) and [`hardware/SHOR15.md`](hardware/SHOR15.md).
...
- A real-QPU `N=15`, `a=2` toy Shor run recovered factors `(3,5)` from measured order-finding data using a 4-qubit modular work register plus one recycled phase ancilla.
