...
On `ibm_fez` with four phase bits:
...
| Shots on the four ideal order-4 peaks | 87.89% | **98.05%** |

Both architectures recovered factors `(3,5)`. The recycled circuit used 37.5% fewer simultaneous logical qubits, about 18% less compiled depth, and about 37% fewer CZ gates. Wide was numerically ahead in direct factor recovery by 6.64 percentage points, but at 256 shots per architecture the separation is only about 1.58 standard errors and is not statistically decisive. The recycled spectrum was substantially cleaner, but it overpopulated the uninformative `0000` order branch.

The correct takeaway from this first baseline is therefore a **resource/width advantage with statistically compatible factoring success**, not a demonstrated success-rate advantage. A stronger controlled comparison should run both circuits in one job while pinning the same four physical work qubits.
...
See [`results/hardware/RESULTS.md`](results/hardware/RESULTS.md), [`results/hardware/shor15_compare.csv`](results/hardware/shor15_compare.csv), and [`hardware/SHOR15.md`](hardware/SHOR15.md).
...
End-to-end real-QPU toy Shor example:
...
- Both real-QPU `N=15`, `a=2` Shor implementations recovered `(3,5)`; recycled used fewer qubits/depth/CZs while the first 256-shot wide-vs-recycled factor-recovery difference was not statistically decisive.
...
Hardware results depend on backend calibration, routing, queue state, Qiskit version, and physical-qubit selection. The current Shor15 wide and recycled runs were close in time and strongly overlapped in physical region, but were separate jobs and were not forced to use an identical work-register layout. A publication-quality comparison should pin shared work qubits and submit both circuits together.
